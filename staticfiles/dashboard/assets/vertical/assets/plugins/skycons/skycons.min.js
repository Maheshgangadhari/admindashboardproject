! function(t) {
    ...
    function f(t, n, i, e, a, o, r, l, s) {
        ...
        if (t.strokeStyle = s, t.lineWidth = o, t.lineCap = "round", t.lineJoin = "round", 1 > m) {
            if (t.beginPath(), m *= d.length / 2 - 1, h = Math.floor(m), m -= h, h *= 2, h += 2, t.moveTo(i + (d[h - 2] * (1 - m) + d[h] * m) * a, e + (d[h - 1] * (1 - m) + d[h + 1] * m) * a), 1 > g) {
                ...
                for (f = h; f !== d.length; f += 2) t.lineTo(i + d[f] * a, e + d[f + 1] * a);
            t.stroke()
        } else if (1 > g) {
    ...
    b.CLEAR_DAY = function(t, n, i) {
        ...
        var s = Math.cos(n * p) * r * .02,
            h = Math.cos((n + .25) * p) * r * .02,
            c = Math.cos((n + .5) * p) * r * .02,
            v = Math.cos((n + .75) * p) * r * .02,
            f = .936 * o,
            d = Math.floor(f - .5 * l) + .5,
            m = Math.floor(f - 2.5 * l) + .5;
        ...
        set: function(t, n) {
            ...
            for ("string" == typeof t && (t = document.getElementById(t)), i = this.list.length; i--;)
                if (this.list[i].element === t) return this.list[i].drawing = this._determineDrawingFunction(n), void this.draw(this.list[i], g);
            this.add(t, n)
        },
        remove: function(t) {
            var n;
